...
import pandas as pd
import numpy as np
import json # 被 json.loads 隐式使用
from sklearn.feature_extraction.text import TfidfVectorizer
import gc
from sklearn.metrics import f1_score, accuracy_score # accuracy_score 被导入但未使用
from sklearn.model_selection import StratifiedKFold, KFold # StratifiedKFold 被导入但 KFold 被使用
...
model = SGDClassifier(n_iter_no_change=10, max_iter=1500, tol=1e-5, loss="modified_huber", random_state=2024)
target_col = 'label'
cv = KFold(5, shuffle=True, random_state=2024)  # KFold
val_scores, test_preds, oof_preds = cross_validate_score(model, train_df[feats], train_df[target_col], cv,
                                                         test_df[feats])
